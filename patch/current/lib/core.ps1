# PATCHED for Stealth mode - Invoke-Git modified to ignore uncommitted changes from patches
...
# PATCHED: Load system.ps1 early to ensure patched environment variable functions are available
# This ensures that any code using Set-EnvVar, Add-Path, or Remove-Path uses our process-only implementations
...
function Invoke-Git {
    ...
    # PATCHED: Disable uncommitted changes check in Invoke-Git
    # Purpose: Allow Scoop updates to work even when our patches are present in the Git repo
    # Original code checks for uncommitted changes via 'git diff' and blocks updates if changes are detected
    # This patch makes 'git diff HEAD --name-only' and similar commands return empty (null),
    # allowing Scoop updates to proceed even though we've patched files in the repo
    if ($ArgumentList -and $ArgumentList.Count -ge 2 -and 
        $ArgumentList[0] -eq 'diff' -and 
        ($ArgumentList[1] -eq 'HEAD' -or $ArgumentList[1] -eq '--cached') -and
        ($ArgumentList -contains '--name-only' -or $ArgumentList -contains '--name-status')) {
        return $null
    }
...
function installed($app, [Nullable[bool]]$global) {
    # PATCHED: Guard against empty app names (custom manager sometimes calls installed('')).
    # Without this check, Select-CurrentVersion receives an empty -AppName and Scoop exits.
    ...
    # PATCHED: Bucket qualified names can produce empty strings after trimming; avoid
    # calling Select-CurrentVersion with an empty app name.
    if ([string]::IsNullOrWhiteSpace($app)) {
        return $false
    }
...
function Invoke-ExternalCommand {
    ...
    if ($LogPath) {
        if ($FilePath -match '^msiexec(.exe)?$') {
            # PATCHED: Split /lwe and log path into separate arguments for proper handling
            # This ensures the argument splitting logic can properly process the log path
            $ArgumentList += '/lwe'
    ...
    if ($ArgumentList.Length -gt 0) {
        ...
        # PATCHED: Fix argument splitting to handle paths with hyphens/spaces (e.g., "D:\- - test\...")
        # Skip splitting for drive-letter paths, TARGETDIR arguments, already-quoted arguments, and 7z filter patterns
        $newArgumentList = @()
        foreach ($arg in $ArgumentList) {
            $originalArg = $arg
            
            # Check if this is a switch with a quoted argument combined (e.g., '/lwe "path"')
            if ($arg -match '^(/[a-z]+)\s+"(.+)"$') {
                # Split switch and quoted path into separate arguments
                $switch = $Matches[1]
                $quotedPath = "`"$($Matches[2])`""
                $newArgumentList += $switch
                $newArgumentList += $quotedPath
                continue
            }
            
            # Check if this argument is already quoted (starts and ends with quotes)
            $isQuoted = $arg -match '^".*"$'
            
            # Remove quotes for processing, but remember if it was quoted
            $arg = $arg -replace '^"|"$', ''
            
            # Don't split if:
            # 1. Already quoted (preserve quoted arguments as-is)
            # 2. Drive-letter paths (e.g., "D:\- - test\...")
            # 3. TARGETDIR arguments
            # 4. 7z filter patterns (e.g., "-ir!...", "-o...")
            if ($isQuoted -or $arg -match '^[A-Z]:[\\/]' -or $arg -match '^TARGETDIR=' -or $arg -match '^-[iox][rx]!' -or $arg -match '^-o') {
                # If it was quoted, restore quotes; otherwise use processed version
                if ($isQuoted) {
                    $newArgumentList += $originalArg
                } else {
                    $newArgumentList += $arg
                }
            } else {
                # Only split arguments that are not paths, TARGETDIR, or filter patterns
                $splitArgs = $arg -split '(?<=(?<![:\w])[/-]\w+) | (?=[/-])'
                $newArgumentList += $splitArgs
            }
        }
        ...
        # PATCHED: Force msiexec to use Arguments property (not ArgumentList) to ensure proper quoting
        # msiexec requires special quoting for paths with hyphens/spaces, which only works with Arguments property
        $ForceLegacyEscaping = $FilePath -match '^msiexec'
        if ((-not $LegacyCommand) -and $SupportArgumentList -and -not $ForceLegacyEscaping) {
            ...
            # PATCHED: Enhanced quoting for msiexec to handle paths with hyphens/spaces correctly
            # msiexec requires paths to be properly quoted with straight double quotes (") to avoid error 1639
            # Reference: https://stackoverflow.com/questions/38010690/msi-installation-throwing-1639-as-exit-error-code-for-msi-having-space-between-c
            # Key points: Use straight double quotes ("), path must be separate argument, paths with spaces/hyphens must be quoted
            $escapedArgs = switch -regex ($ArgumentList) {
                # Skip already-quoted arguments (preserve as-is)
                { $_ -match '^".*"$' } { $_; continue }
                # PATCHED: Handle TARGETDIR=path format - quote the path part if it contains hyphens or spaces
                # This handles cases like TARGETDIR=D:\- - test\... which msiexec misinterprets
                { $FilePath -match '^msiexec' -and $_ -match '^TARGETDIR=' } {
                    if ($_ -match '^TARGETDIR=(.+)$') {
                        $pathPart = $Matches[1]
                        # Quote the path if it contains hyphens or spaces
                        if ($pathPart -match '[-\s]') {
                            "TARGETDIR=`"$pathPart`""
                        } else {
                            $_
                ...
                # Quote paths starting with a drive letter (but not if part of TARGETDIR= or /D=)
                '(?<!/D=)(?<!TARGETDIR=)[A-Z]:[\\/].*' { $_ -replace '([A-Z]:[\\/].*)', '"$1"'; continue }
                # Do not quote paths if it is NSIS's '/D' argument
                '/D=[A-Z]:[\\/].*' { $_; continue }
                # Quote args with spaces
                ' ' { "`"$_`""; continue }
                # PATCHED: For msiexec, also quote arguments that contain hyphens (to prevent misinterpretation)
                { $FilePath -match '^msiexec' -and $_ -match '-' -and $_ -notmatch '^[A-Z]:' -and $_ -notmatch '^TARGETDIR=' } { "`"$_`""; continue }
                default { $_; continue }
            }
