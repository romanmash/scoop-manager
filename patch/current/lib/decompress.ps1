# PATCHED for Stealth mode - Expand-MsiArchive modified to quote paths with spaces
...
function Expand-MsiArchive {
    ...
    if ((get_config USE_LESSMSI)) {
        ...
        $MsiPath = 'msiexec.exe'
        # PATCHED: Fix MSI installation with paths containing spaces and special characters
        # Purpose: Fix Windows Installer error 1639 (Invalid command line argument) when paths contain spaces/hyphens
        # Original code passes clean argument array: @('/a', $Path, ...) - this is correct
        # Solution: Always pass clean argument array with separate tokens - let Invoke-ExternalCommand handle quoting
        #   - Invoke-ExternalCommand automatically detects and quotes drive paths containing hyphens/spaces
        #   - Invoke-ExternalCommand detects spaces and quotes arguments containing spaces
        #   - Do NOT combine switches and paths into single strings - this interferes with proper argument parsing
        # NOTE: Do NOT pack "/a $Path" into a single string. Invoke-ExternalCommand performs its own argument
        #   splitting and quoting. Combining the switch and path can cause invalid msiexec command lines
        #   (e.g. under roots like "D:\- - test\portable_scoop"), leading to exit code 1639.
