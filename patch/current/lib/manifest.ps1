# PATCHED for VirusTotal app@version support (version-specific manifests)
...
function Get-Manifest($app) {
    ...
    if ($app -match '^(ht|f)tps?://|\\\\') {
        ...
        if (installed $app) {
            ...
            if ($bucket -and $version) {
                # Version-specific request with explicit bucket (bucket/app@version)
                # Prefer a generated per-version manifest; fall back to bucket manifest.
                $generatedPath = generate_user_manifest $app $bucket $version
                if ($generatedPath -and (Test-Path $generatedPath)) {
                    $manifest = parse_json $generatedPath
                } else {
                    $manifest = manifest $app $bucket
                }
            } elseif ($bucket) {
                # Explicit bucket but no version -> use bucket manifest as-is
                $manifest = manifest $app $bucket
            } elseif ($version) {
                # Version-specific request without explicit bucket (app@version).
                # Let generate_user_manifest resolve the appropriate bucket (same
                # behaviour as scoop-install) and then track which buckets contain
                # a manifest for this app for warning purposes.
                $generatedPath = generate_user_manifest $app $null $version
                if ($generatedPath -and (Test-Path $generatedPath)) {
                    $manifest = parse_json $generatedPath
                }

                # Discover buckets that have this app (for warnings / metadata)
                foreach ($tekcub in Get-LocalBucket) {
                    $current_manifest = manifest $app $tekcub
                    if (!$bucket -and $current_manifest) {
                        $bucket = $tekcub
                    }
                    if ($current_manifest) {
                ...
                # No bucket and no version -> original behaviour, pick first matching bucket manifest
                foreach ($tekcub in Get-LocalBucket) {
                    $current_manifest = manifest $app $tekcub
