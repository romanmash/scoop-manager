# PATCHED for Stealth mode - Add-ShimsDirToPath and Write-Env 'PATH' calls disabled
...
function Add-ShimsDirToPath {
    ...
    if ($userEnvPath -notmatch [Regex]::Escape($SCOOP_SHIMS_DIR)) {
        ...
        # PATCHED: Disable registry writes for PATH
        # Purpose: Prevent persistent PATH modifications (stealth mode)
        # Original code writes to registry for future sessions via Write-Env
        # This patch disables registry writes and only updates process-level PATH
        # For future sessions (disabled):
        # Write-Env 'PATH' "$SCOOP_SHIMS_DIR;$userEnvPath"
        # For current session (process-level only):
...
function Install-Scoop {
    ...
    Import-ScoopShim
    # PATCHED: Disable Add-ShimsDirToPath call
    # Purpose: Prevent persistent PATH modifications (stealth mode)
    # Original code adds shims directory to user PATH registry for future sessions
    # This patch disables the call to maintain stealth mode - shims are added to process PATH only
    # (via env var in Add-ShimsDirToPath function)
    # Finally ensure scoop shims is in the PATH (disabled):
    # Add-ShimsDirToPath
